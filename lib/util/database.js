...
const Database = {
  fetchHighscores(database) {
    ...
    const onFetch = database.ref(`leaderboard/`).on('value', (snapshot) => {
      leaders = snapshot.val();
      sortedLeaders = leaderCompare(leaders);
      populateLeaderboard(sortedLeaders, leaders);
    });

    database.ref(`leaderboard/`).off('value', onFetch);
  },
  ...
  setHighscore(database, score) {
    ...
    const onChange = database.ref(`leaderboard/`).on('value', (snapshot) => {
      leaders = snapshot.val();
      sortedLeaders = leaderCompare(leaders);
    ...
    database.ref(`leaderboard/`).off('value', onChange);
  },
};
